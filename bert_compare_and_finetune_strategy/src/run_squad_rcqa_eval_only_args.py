...
def evaluate(args, model, tokenizer, use_threshold=True, prefix=""):
    ...
    if use_threshold:
        ...
        # preds = json.load(open(output_prediction_file, 'rb'))

        # load the null score differences we generated earlier
        null_odds = json.load(open(output_null_log_odds_file, 'rb'))

        results_default_thresh = squad_evaluate(examples, 
                                                predictions, 
                                                no_answer_probs=null_odds, 
                                                no_answer_probability_threshold=0)
        return results_default_thresh
...
class ParserArg():
  def __init__(self,
               data_dir='/home/ubuntu/bert_compare/data/rcqa/laboro_jumandic',
               model_type='bert',
               model_name_or_path='/home/ubuntu/bert_compare/model/webcorpus_large_bpe_cf',
    ...
    #self.output_dir=os.path.join(self.output_dir,'checkpoint-5400')
...
if __name__=='__main__':
    ...
    if args.num_train_epochs == 2:
        args.output_dir=os.path.join(args.output_dir,'checkpoint-5400')
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # Evaluate
            result = evaluate(args, model, tokenizer, use_threshold=False, prefix=f'dev_{global_step}')

            result = dict((k + ("_{}".format(global_step) if global_step else ""), v) for k, v in result.items())
            results.update(result)

    logger.info("Results: {}".format(results))
    output_eval = os.path.join(args.output_dir,'dev_eval_results.json')
    json.dump(results,open(output_eval,'w'))
    
    
    best_dev_f1_threshold = json.load(open(output_eval))['best_f1_thresh']
    
    
    # dev using null threshold
    args.null_score_diff_threshold = best_dev_f1_threshold
    ...
    logger.info("Results: {}".format(results))
    output_eval = os.path.join(args.output_dir,'devth_eval_results.json')
    json.dump(results,open(output_eval,'w'))
    
    
    # test no using null threshold
    args.null_score_diff_threshold=0
    args.predict_file='test-v1.0.json'
    ...
    logger.info("Results: {}".format(results))
    output_eval = os.path.join(args.output_dir,'test_eval_results.json')
    json.dump(results,open(output_eval,'w'))
    
    
    # test using null threshold
    args.null_score_diff_threshold=best_dev_f1_threshold
    args.predict_file='test-v1.0.json'
    ...
    logger.info("Results: {}".format(results))
    output_eval = os.path.join(args.output_dir,'testth_eval_results.json')
    json.dump(results,open(output_eval,'w'))